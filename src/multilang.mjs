 ...
 * @returns {string}
 */
export function buildEnglishSystemPrompt(faqData) {
  const faqText = buildFaqText(faqData);
  const faqCount = faqData.length;

  return `You are the AI Customer Support assistant for Sloten (sloten.io).
You assist customers who use the Sloten online casino platform.
...
If you detect keywords related to: addiction, debt, financial hardship, self-harm, or suicidal thoughts:
→ Show empathy, provide helpline information, and immediately escalate to a human agent
- Consumer Hotline (Japan): 188
- Yorisoi Hotline: 0120-279-338
- For international users, recommend consulting a local problem gambling helpline
 ...
 * @returns {string}
 */
export function buildJapaneseSystemPrompt(faqData) {
  const faqText = buildFaqText(faqData);
  const faqCount = faqData.length;

  return `あなたはスロット天国（Sloten）のAIカスタマーサポートです。
 ...
 */
export async function buildLocalizedContext(env, userMessage) {
  ...
  const systemPrompt = language === 'en'
    ? buildEnglishSystemPrompt(faqData)
    : buildJapaneseSystemPrompt(faqData);
...
// 英語RGキーワード
const RG_KEYWORDS_EN = /\b(addict|addiction|can't stop|cannot stop|gambling problem|debt|living expenses|help me|kill myself|suicide|suicidal|depressed|hopeless|want to die|end my life)\b/i;
...
// 英語怒りパターン
const ANGER_PATTERNS_EN = /\b(scam|fraud|rip.?off|bullshit|garbage|worst|terrible|sue you|lawyer|refund|give .* back|how many times|ridiculous|unacceptable|disgusting)\b/i;
